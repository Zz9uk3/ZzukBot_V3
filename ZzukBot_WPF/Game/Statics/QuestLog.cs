...
using ZzukBot.Game.Frames.FrameObjects;
using ZzukBot.Mem;
...
namespace ZzukBot.Game.Statics
{
    ...
    public sealed class QuestLog
    {
            ...
            for (var i = 0; i < totalEntrys; i++)
            {
                ...
                else
                {
                    ...
                    if (oldEntry.State != questEntry.State)
                        ...
                        {
                            case Enums.QuestState.Completed:
                                OnQuestComplete?.Invoke(questEntry, new EventArgs());
                                break;

                            case Enums.QuestState.Failed:
        ...
        private static Enums.QuestState QuestStateByIndex(int questIndex)
        {
            ...
            if (failed) return Enums.QuestState.Failed;
            if (int1 == 0 && (0xBB71CC + shiftedQuestIndex).ReadAs<int>() != 0) return Enums.QuestState.Failed;
            if (Functions.CanCompleteQuest(questIndex)) return Enums.QuestState.Completed;
            return Enums.QuestState.InProgress;
        }
        ...
        private static QuestObjective[] GetQuestObjectives(IntPtr questCacheRowPtr, int realIndex)
        {
            ...
            if (eventObjective.ReadAs<int>() != 0)
            {
                ...
                var tmp = new QuestObjectiveInterface(Enums.QuestObjectiveTypes.Event, 0, 0, 0, eventIsDone);
                objectives.Add(tmp);
            }
            // ################################
            // ##########Kill#Objective########
            ...
            while (objectiveIndex < 4)
            {
                ...
                if (killObjective.ReadAs<int>() != 0)
                {
                    ...
                    var tmp = new QuestObjectiveInterface(Enums.QuestObjectiveTypes.Kill, killObjectiveId,
                ...
                else
                {
                    ...
                    if (itemObjective.ReadAs<int>() != 0)
                    {
                        ...
                        var tmp = new QuestObjectiveInterface(Enums.QuestObjectiveTypes.Collect, itemObjectiveId,
        ...
        {
            internal QuestObjectiveInterface(Enums.QuestObjectiveTypes parType, int parObjectId,
